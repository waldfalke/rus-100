...
export interface PaymentOption {
  ...
  label: string; // Текст ссылки (напр., "Оплатить на месяц")
  description?: string; // Описание периода (напр., "до 16.05.2025")
  cost: number; // Стоимость в рублях (напр., 3000)
  savings?: number; // Сумма экономии при выборе этого варианта
  url?: string; // URL для перехода к оплате (пока заглушка '#')
  period?: 'month' | 'year'; // Тип периода
}
