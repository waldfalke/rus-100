...
### Preconditions
- Данные о доступных вопросах (массив `questions`) должны быть загружены или доступны для загрузки (с учетом пагинации и фильтров).
- Должны быть переданы функции обратного вызова для обработки действий пользователя: `handleSearchChange`, `handleDifficultyChange`, `handleTypeChange`, `handleClearFilters`, `handleQuestionSelect`, `handleQuestionCheckbox`, `handlePageChange`, `handleAddSelectedToTest`, `handleClearSelection`.
- Должны быть переданы состояния: `searchQuery`, `difficultyFilter`, `typeFilter`, `selectedQuestions`, `expandedQuestions`, `currentPage`, `pageSize`, `totalPages`, `totalQuestions`, `isLoading`.
- Маппинги `questionTypeLabels` и `difficultyLabels` должны быть доступны.

### Postconditions
- **После применения фильтров/поиска/пагинации:**
    - Список отображаемых вопросов (`questions`) обновляется в соответствии с заданными критериями и текущей страницей.
    - Обновляется информация пагинации (`currentPage`, `totalPages`, `totalQuestions`).
    - Отображается состояние загрузки (`isLoading`) во время запроса данных.
- **После выбора/снятия выбора вопроса (клик/чекбокс):**
    - `id` вопроса добавляется/удаляется из массива `selectedQuestions`.
    - Визуальное состояние карточки вопроса обновляется.
    - Панель выбранных вопросов отображается/скрывается и обновляет счетчик.
- **После клика "Добавить в тест":**
    - Вызывается `handleAddSelectedToTest` со списком `selectedQuestions`.
    - Список `selectedQuestions` может быть очищен (в зависимости от логики `handleAddSelectedToTest`).
- **После клика "Очистить выбор":**
    - Вызывается `handleClearSelection`, массив `selectedQuestions` очищается.
    - Панель выбранных вопросов скрывается.

### Invariants
- Список отображаемых вопросов всегда соответствует текущим значениям фильтров (`searchQuery`, `difficultyFilter`, `typeFilter`) и пагинации (`currentPage`, `pageSize`).
- Массив `selectedQuestions` всегда содержит только `id` вопросов, которые были выбраны пользователем.
- Состояние `aria-selected` для карточки вопроса и `checked` для чекбокса всегда соответствует наличию `question.id` в `selectedQuestions`.
- Панель выбранных вопросов отображается тогда и только тогда, когда `selectedQuestions.length > 0`.
- Пагинация корректно отображает текущую страницу и общее количество страниц/вопросов.
- Кнопки пагинации ("Предыдущая"/"Следующая") неактивны на первой/последней странице соответственно.

### Exceptions
- **Ошибка загрузки данных**: Не удалось загрузить список вопросов с сервера (сетевая ошибка, ошибка API).
- **Ошибка состояния**: Не переданы необходимые состояния или функции обратного вызова.
- **Некорректные данные**: Получены некорректные данные о вопросах (отсутствуют обязательные поля, неверные типы).
- **Ошибка фильтрации/поиска/пагинации**: Логика на стороне клиента или сервера работает некорректно.
- **Ошибка коллбэка**: Функция, переданная в `on...` пропсах, вызывает ошибку.
- **Рассинхронизация состояния**: Отображаемый список вопросов или состояние выбора не соответствует реальному состоянию.
