...
### Preconditions
- Данные о результатах завершенного теста (общий балл, время, ответы на каждый вопрос, правильные ответы, пояснения и т.д.) должны быть доступны и переданы компоненту.
- Данные должны быть корректны и консистентны (например, сумма баллов за вопросы должна сходиться с общим баллом).
- Идентификаторы (`testId`, `userId`, `attemptId`) должны быть валидными.
- Пользователь (ученик или преподаватель) должен быть аутентифицирован и иметь права на просмотр данных результатов.

### Postconditions
- Корректно отображается общая статистика теста (балл, процент, время, количество правильных/неправильных ответов).
- Отображается список всех вопросов теста с указанием статуса ответа (верно/неверно/пропущено) и набранных баллов.
- При клике на вопрос раскрывается детальная информация (текст вопроса, ответ пользователя, правильный ответ, пояснение).
- Работает фильтрация и сортировка списка вопросов.
- Корректно отображаются графики/диаграммы (например, распределение по категориям), если предусмотрено.
- Успешно выполняются функции экспорта (PDF) и печати результатов (при вызове).
- Данные о просмотре результатов могут быть залогированы.

### Invariants
- Отображаемая статистика (общий балл, процент, кол-во ответов) всегда соответствует переданным данным о результатах.
- Сумма баллов за отдельные вопросы всегда равна общему баллу (`score`).
- Статус ответа на вопрос (верно/неверно/пропущено) всегда соответствует сравнению ответа пользователя с правильным ответом.
- Отображаемые правильные ответы и пояснения соответствуют данным из банка вопросов.
- Фильтрация/сортировка не изменяет исходные данные о результатах, а только их представление.
- Данные о результатах (баллы, ответы) не могут быть изменены через интерфейс просмотра.

### Exceptions
- **Ошибка доступа**: Попытка просмотра результатов без аутентификации или необходимых прав.
- **Некорректные/Неполные данные**: Отсутствие обязательных данных о результатах, неконсистентность данных (например, не сходятся баллы). Компонент может отобразить ошибку или неполную информацию.
- **Ошибка рендеринга**: Проблемы при отображении сложных элементов (графики, форматированный текст вопросов/пояснений).
- **Ошибка экспорта/печати**: Сбой при генерации PDF или формировании версии для печати.
- **Ошибка фильтрации/сортировки**: Некорректная работа логики фильтрации или сортировки.
- **Сетевая ошибка**: Если данные подгружаются динамически, возможны ошибки при загрузке.
