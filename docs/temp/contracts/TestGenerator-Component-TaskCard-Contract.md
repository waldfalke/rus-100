...
### Preconditions
- Обязательные пропсы `id`, `title`, `count`, `maxCount`, `difficultyType`, `onCountChange` должны быть переданы.
- `count` должен быть `>= 0` и `<= maxCount`.
- `maxCount` должен быть `>= 0`.
- `difficultyType` должен быть одним из: "none", "easy", "hard", "both".
- Если `difficultyType` не "none":
    - Проп `difficulty` должен быть передан и быть одним из: "easy", "hard", "both", `null`.
    - Проп `onDifficultyChange` должен быть передан.

### Postconditions
- При клике на кнопку "+":
    - Если `count < maxCount`, вызывается `onCountChange(id, 1)`.
    - Визуально обновляется отображаемое значение `count`.
    - Кнопка "-" становится активной (если была неактивна).
    - Кнопка "+" становится неактивной, если `count` достиг `maxCount`.
- При клике на кнопку "-":
    - Если `count > 0`, вызывается `onCountChange(id, -1)`.
    - Визуально обновляется отображаемое значение `count`.
    - Кнопка "+" становится активной (если была неактивна).
    - Кнопка "-" становится неактивной, если `count` достиг 0.
- При клике на селектор сложности (если `difficultyType !== 'none'`):
    - Вызывается `onDifficultyChange(id, newDifficulty)` с циклически измененным значением сложности.
    - Визуально обновляется индикатор сложности (текст и цвет).

### Invariants
- Отображаемый `count` всегда соответствует значению пропа `count` и находится в диапазоне `[0, maxCount]`.
- Кнопка "-" неактивна (`disabled`) тогда и только тогда, когда `count === 0`.
- Кнопка "+" неактивна (`disabled`) тогда и только тогда, когда `count === maxCount`.
- Селектор сложности отображается тогда и только тогда, когда `difficultyType !== 'none'`.
- Визуальное представление индикатора сложности (текст и цвет) всегда соответствует значению пропа `difficulty`.
- `id` и `title` карточки не изменяются.

### Exceptions
- **Некорректные пропсы**: Отсутствие обязательных пропсов, передача значений неправильного типа или вне допустимых диапазонов (e.g., `count > maxCount`, невалидный `difficultyType`). Компонент может не рендериться или работать некорректно.
- **Отсутствие обработчика**: Если `difficultyType !== 'none'`, но `onDifficultyChange` не передан, клик по селектору сложности не будет иметь эффекта или вызовет ошибку.
- **Ошибка коллбэка**: Если `onCountChange` или `onDifficultyChange` вызывают ошибку в родительском компоненте.
- **Ошибка стилизации**: Если Tailwind классы не определены или переопределены, визуальное представление будет нарушено.
