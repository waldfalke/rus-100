...
### Preconditions
- Компонент `TaskCardBlock.tsx` должен быть реализован согласно B-варианту (`TaskCardBlockB.stories.tsx`).
- Функция `renderItemRow` в родительском компоненте (`TestGenerator2`) должна корректно подготавливать и передавать пропсы.
- Данные `item` (с `id`, `title`), `category`, `tabType`, `difficultyStatsData`, `mockDifficultyTiers` должны быть доступны и корректны в `renderItemRow`.
- Пропсы `currentCount` и `maxCount` должны быть неотрицательными числами, `currentCount <= maxCount`.
- Коллбэки (`handleCountChange`, `handleItemDifficultyChange`, `handleCategoriesChange`) должны быть валидными функциями.
- Вспомогательные функции `createDifficultyDropdown` и `createCategoryDropdown` должны быть доступны и корректно реализованы, если используются.

### Postconditions
- Карточка корректно рендерит один из трех B-вариантов (`Full Card B`, `Only Difficulty B`, `Minimal B`) на основе `taskNumber` и `itemStats`.
- Отображаются соответствующие элементы управления: dropdown-ы сложности и/или категорий (если применимо) и счетчик (+/-).
- Визуальное представление соответствует указанным Tailwind классам и скриншотам B-варианта.
- Нажатия на +/- вызывают `handleCountChange` с корректными параметрами.
- Выбор в dropdown-ах сложности/категорий вызывает `handleItemDifficultyChange`/`handleCategoriesChange`.
- Общий прогресс в родительском компоненте обновляется при изменении счетчика.

### Invariants
- Счетчик `currentCount` всегда находится в диапазоне `[0, maxCount]`.
- Кнопки +/- всегда отображаются.
- Тип карточки определяется **только** по наличию `taskNumber` (из `item.title`) и `itemStats`.
- Если `currentCount` равно 0, кнопка "-" неактивна (`disabled`).
- Если `currentCount` равно `maxCount`, кнопка "+" неактивна (`disabled`).
- При отсутствии `taskNumber` элементы управления категориями не отображаются.
- При отсутствии `itemStats` элементы управления сложностью не отображаются.
- Визуальное состояние элементов (Badge, кнопки) соответствует их текущему статусу (выбран/не выбран, активна/неактивна).
- Адаптивность: компоновка и стили корректно изменяются на мобильных устройствах согласно CSS правилам.

### Exceptions
- **Ошибка определения типа**: Некорректный формат `item.title` или отсутствие `itemStats` приведет к неправильному выбору варианта карточки.
- **Ошибка рендеринга**: Отсутствие обязательных данных или коллбэков в `renderItemRow` может привести к сбою рендеринга `TaskCardBlock`.
- **Ошибка коллбэка**: Исключения в функциях `handle...Change` могут нарушить обновление состояния.
- **Визуальное несоответствие**: Если верстка отклоняется от заданных классов и правил адаптивности.
- **Ошибка зависимостей**: Некорректная работа `Popover` или других базовых UI компонентов.
