...
### Preconditions
- Массив или список вкладок (например, ['tasks', 'ege', 'exercises']) должен быть определен.
- Функция (`setActiveTab`) для обновления состояния активной вкладки должна быть передана.
- Начальное значение активной вкладки (`activeTab`) должно быть установлено.
- Контент для каждой вкладки (например, компоненты, соответствующие `panel-tasks`, `panel-ege`, `panel-exercises`) должен быть доступен для рендеринга.

### Postconditions
- При клике на неактивную, доступную вкладку вызывается `setActiveTab` с `id` этой вкладки.
- Визуально выделяется новая активная вкладка (стили `border-indigo-500 text-indigo-600`).
- Отображается контент (`div` с соответствующим `id`), связанный с новой активной вкладкой.
- Контент неактивных вкладок скрыт (например, через класс `hidden`).
- Состояние `activeTab` успешно обновляется в родительском компоненте.

### Invariants
- Всегда активна ровно одна вкладка (ее `id` соответствует `activeTab`).
- Порядок вкладок в панели остается неизменным.
- Атрибуты доступности (`role`, `aria-selected`, `aria-controls`) всегда корректно отражают текущее состояние.
- Отключенные вкладки (если бы они были, судя по состояниям) не реагируют на клики и имеют `aria-disabled="true"`.
- Визуальное представление активной/неактивной вкладки соответствует Tailwind классам, указанным в контракте.

### Exceptions
- **Ошибка состояния:** Не передана функция `setActiveTab` или начальное значение `activeTab`.
- **Некорректный ID вкладки:** Попытка установить `activeTab` в значение, не соответствующее ни одной из определенных вкладок.
- **Ошибка рендеринга контента:** Компонент, отображаемый внутри активной вкладки, вызывает ошибку.
- **Нарушение доступности:** Отсутствие или некорректное использование ARIA-атрибутов, отсутствие поддержки клавиатурной навигации.
- **Проблемы стилизации/адаптивности:** Некорректное отображение вкладок или их трансформация на мобильных устройствах.
