...
### Preconditions
- **Для входа (`login`):**
    - Пользователь должен предоставить корректные `email` и `password`.
    - Учетная запись с таким `email` должна существовать и быть активной.
    - Сервис аутентификации должен быть доступен.
- **Для получения/обновления профиля (`getUserProfile`, `updateUserProfile`):**
    - Пользователь должен быть аутентифицирован (иметь валидный токен доступа).
    - Запрашиваемый `userId` (или `profileId`) должен существовать и принадлежать аутентифицированному пользователю.
    - При обновлении предоставляемые данные должны соответствовать схеме `User`/`Profile` и проходить валидацию.
- **Для переключения профиля (`switchProfile`):**
    - Пользователь должен быть аутентифицирован.
    - Запрашиваемый `profileId` должен существовать и принадлежать данному пользователю.
- **Для создания профиля:**
    - Пользователь должен быть аутентифицирован.
    - Предоставленные данные для нового профиля должны быть валидны (например, уникальное имя профиля для пользователя).

### Postconditions
- **После успешного входа (`login`):**
    - Система возвращает данные пользователя (`User`) и токен доступа.
    - Создается активная сессия пользователя.
    - Загружается профиль по умолчанию или последний использованный.
    - Записывается время последнего входа (`lastLoginAt`).
- **После успешного выхода (`logout`):**
    - Сессия пользователя завершается, токен доступа становится недействительным.
- **После успешного обновления профиля (`updateUserProfile`):**
    - Данные пользователя/профиля в БД обновлены.
    - Поле `updatedAt` обновлено.
    - Обновленные данные возвращаются пользователю.
- **После успешного переключения профиля (`switchProfile`):**
    - Активный профиль пользователя изменен в рамках текущей сессии.
    - Интерфейс пользователя обновляется для отражения настроек нового профиля.
    - Выбор профиля сохраняется для будущих сессий.
- **После успешного создания профиля:**
    - В БД создается новая запись `Profile`, связанная с `userId`.
    - Новый профиль становится доступным для выбора в `ProfileSwitcher`.

### Invariants
- **Уникальность Email**: Поле `email` в таблице `Users` всегда уникально.
- **Целостность данных**: `userId` в `Profile` всегда ссылается на существующего пользователя.
- **Наличие профиля по умолчанию**: У каждого пользователя всегда есть хотя бы один профиль, и ровно один из них помечен как `isDefault=true` (или используется логика выбора последнего активного).
- **Состояние аутентификации**: Пользователь может выполнять действия, требующие аутентификации, только при наличии валидной сессии/токена.
- **Связь пользователь-профиль**: Профили, доступные для переключения, всегда принадлежат текущему аутентифицированному пользователю.

### Exceptions
- **Ошибка аутентификации**: Неверный `email` или `password` при входе.
- **Ошибка авторизации**: Попытка доступа к чужому профилю или выполнения действия без необходимых прав.
- **Невалидные данные**: Попытка обновить/создать профиль с некорректными данными (нарушение уникальности имени профиля, неверный формат и т.д.).
- **Несуществующий ресурс**: Попытка получить/обновить/переключить несуществующий `userId` или `profileId`.
- **Ошибка сессии**: Истекший или недействительный токен доступа.
- **Ошибка БД**: Проблемы при чтении/записи данных пользователей или профилей.
- **Сетевая ошибка**: Проблемы при взаимодействии с сервисом аутентификации/управления пользователями.
