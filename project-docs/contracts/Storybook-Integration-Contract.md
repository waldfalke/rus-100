...
### Preconditions
- Существует репозиторий проекта с кодовой базой на Next.js + TypeScript.
- Установлены необходимые инструменты: Node.js, npm/yarn/pnpm.
- Доступны права для установки зависимостей и настройки CI/CD (если требуется).
- Определена базовая структура папки компонентов (e.g., `components/`).

### Postconditions
- Storybook успешно установлен и сконфигурирован в проекте (`npx storybook@latest init ...`).
- Storybook запускается локально (`npm run storybook`) и отображает UI-компоненты.
- Создана структура для stories и документации (e.g., `MyComponent.stories.tsx`, `MyComponent.md`).
- Настроен CI/CD для автоматической сборки (`npm run build-storybook`) и деплоя Storybook.
- Опубликованный Storybook доступен по URL для команды.
- Разработчики проинструктированы о правилах создания stories и документации.

### Invariants
- **Покрытие компонентов**: Все новые UI-компоненты, добавляемые в библиотеку, **должны** сопровождаться соответствующими `.stories.tsx` и файлом документации/контракта.
- **Актуальность**: При изменении компонента (пропсов, поведения, визуала) его stories и документация **должны** быть обновлены в том же Pull Request.
- **Доступность**: Развернутый Storybook всегда доступен команде по актуальному URL.
- **Консистентность**: Структура stories и документации соответствует принятым в контракте стандартам.
- **CI/CD**: Процесс автоматической сборки и деплоя Storybook функционирует корректно.

### Exceptions
- **Ошибка установки/конфигурации**: Невозможность установить Storybook или настроить его интеграцию (например, с TailwindCSS, TypeScript).
- **Ошибка сборки/деплоя**: CI/CD пайплайн падает при сборке или деплое Storybook.
- **Несоответствие компонентов**: Компоненты отображаются в Storybook некорректно из-за проблем с конфигурацией или спецификой компонента.
- **Нарушение инвариантов разработчиками**: Добавление компонентов без stories/документации, неактуальная документация.
- **Проблемы доступа**: Опубликованный Storybook недоступен (проблемы с хостингом, правами доступа).
