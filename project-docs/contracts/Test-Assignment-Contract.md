...
- **Для создания назначения:**
    - Пользователь (преподаватель) должен быть аутентифицирован и иметь права на создание назначений.
    - Выбранный `testId` должен соответствовать существующему и активному тесту.
    - Должен быть указан хотя бы один получатель (`students` или `groups`), и эти идентификаторы должны существовать в системе.
    - Дата окончания (`availableUntil`) должна быть позже даты начала (`availableFrom`).
    - Количество попыток (`attemptsAllowed`) должно быть >= 1.
- **Для начала выполнения теста:**
    - Пользователь (ученик) должен быть аутентифицирован.
    - Назначение (`assignmentId`) должно существовать и быть в статусе `ACTIVE`.
    - Текущее время должно быть в пределах `availableFrom` и `availableUntil`.
    - Количество совершенных попыток учеником должно быть меньше `attemptsAllowed`.

### Postconditions
- **После успешного создания назначения:**
    - В базе данных создается новая запись `Assignment` со статусом `SCHEDULED` или `ACTIVE`.
    - Соответствующие записи создаются в `AssignmentNotification` для отправки уведомлений.
    - Ученики/группы видят новое назначение в своих интерфейсах (`AssignmentViewer`).
- **После успешного начала выполнения:**
    - Создается новая запись `AssignmentAttempt` со статусом `IN_PROGRESS`.
    - Ученику отображается интерфейс прохождения теста.
    - Запускается таймер (если `durationMinutes` установлен).
- **После успешного завершения выполнения:**
    - Статус `AssignmentAttempt` меняется на `COMPLETED` или `TIMED_OUT`.
    - Рассчитывается результат (`score`, `percentage`), ответы сохраняются.
    - Результаты становятся доступны ученику (если `showResults` = true).
    - Отправляется уведомление `COMPLETION` преподавателю.

### Invariants
- **Согласованность статусов**: Статус `Assignment` (`SCHEDULED`, `ACTIVE`, `COMPLETED`) всегда отражает текущее время относительно `availableFrom` и `availableUntil` (с учетом фонового процесса обновления статусов).
- **Целостность данных**: `assignmentId`, `studentId`, `questionId` в `AssignmentAttempt` и `Answer` всегда ссылаются на существующие записи.
- **Ограничение попыток**: Количество записей `AssignmentAttempt` для пары `(assignmentId, studentId)` никогда не превышает `attemptsAllowed`.
- **Контроль доступа**: Только назначенные ученики/группы могут видеть и выполнять назначение. Только создатель (преподаватель) может редактировать/удалять назначение.
- **Неизменность завершенных попыток**: Данные завершенной попытки (`COMPLETED`, `TIMED_OUT`) не изменяются.

### Exceptions
- **Ошибка доступа**: Попытка неавторизованного пользователя создать/изменить/выполнить назначение.
- **Некорректные данные**: Попытка создать назначение с несуществующим `testId`, `studentId`, `groupId` или неверными датами.
- **Нарушение лимита попыток**: Попытка начать новую попытку при исчерпании `attemptsAllowed`.
- **Истекшее время**: Попытка начать выполнение теста вне периода `availableFrom` - `availableUntil`.
- **Ошибка сервиса уведомлений**: Невозможность отправить уведомление (`NotificationService` недоступен или ошибка конфигурации).
- **Ошибка БД**: Нарушение ограничений при записи `Assignment`, `AssignmentAttempt`, `Answer`.
- **Конфликт состояний**: Попытка изменить назначение, которое уже активно используется учениками.
