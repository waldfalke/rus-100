...
### Preconditions
- **Для ручного создания/редактирования теста:**
    - Пользователь (учитель) должен быть аутентифицирован и иметь права на создание/редактирование тестов.
    - Должен быть выбран существующий предмет.
    - Выбранные вопросы (`questionId`) должны существовать в банке заданий и соответствовать выбранному предмету.
    - Указанные баллы за вопросы должны быть в допустимом диапазоне (e.g., [1, 10]).
    - Общее количество вопросов не должно превышать лимит (e.g., 100).
- **Для автоматической генерации теста:**
    - Пользователь (учитель) должен быть аутентифицирован и иметь права на создание тестов.
    - Должен быть выбран существующий предмет.
    - Должны быть указаны критерии генерации (темы/типы/сложность) и общее количество вопросов.
    - Заданное количество вопросов должно быть реализуемо с учетом доступных вопросов в банке и критериев балансировки.
- **Для публикации теста:**
    - Тест должен быть успешно сохранен и провалидирован (`TestValidator`).
    - Тест должен содержать хотя бы один вопрос.

### Postconditions
- **После успешного сохранения теста (ручного или сгенерированного):**
    - В базе данных создается или обновляется запись `Tests`.
    - Создаются или обновляются записи `TestQuestions`, связывающие тест с вопросами и их параметрами (позиция, баллы).
    - Тест доступен для дальнейшего редактирования и назначения.
- **После успешной публикации теста:**
    - Статус теста (`Tests.is_active` или аналогичный) изменяется на "опубликован"/"активен".
    - Тест становится доступным для выбора в компоненте назначения тестов (`TestAssignment`).
- **После успешного удаления теста:**
    - Запись `Tests` и связанные записи `TestQuestions` удаляются из БД.
    - Тест больше не доступен для редактирования или назначения.

### Invariants
- **Связь тест-вопрос**: Каждый вопрос (`TestQuestions.question_id`), добавленный в тест (`TestQuestions.test_id`), существует в таблице `Questions`.
- **Принадлежность предмету**: Все вопросы в тесте относятся к предмету, указанному в `Tests.subject_id`.
- **Уникальность вопросов в тесте**: Одна и та же запись вопроса (`question_id`) не может быть добавлена в один и тот же тест (`test_id`) дважды (гарантируется `PRIMARY KEY (test_id, question_id)`).
- **Корректность баллов/позиций**: Поля `position` и `points` в `TestQuestions` всегда имеют допустимые значения.
- **Статус теста**: Неопубликованный тест не может быть назначен ученикам.

### Exceptions
- **Ошибка доступа**: Попытка неавторизованного пользователя или пользователя без прав "Учитель" создать/изменить/удалить тест.
- **Невалидные данные**: Попытка сохранить тест с несуществующими `subject_id`, `question_id`, недопустимым количеством вопросов или баллов.
- **Ошибка поиска вопросов**: `QuestionSelector` не может найти вопросы по заданным критериям (например, нет вопросов нужной темы/сложности).
- **Ошибка генерации**: `TestGenerator` не может сгенерировать тест из-за нехватки вопросов в банке или конфликтующих критериев балансировки.
- **Ошибка валидации**: `TestValidator` обнаруживает проблемы в тесте (например, некорректный баланс), которые не были устранены.
- **Ошибка БД**: Невозможность записать/обновить/удалить данные в таблицах `Tests`, `TestQuestions` из-за проблем с соединением или нарушения ограничений.
- **Конфликт редактирования**: Попытка изменить тест, который уже назначен и выполняется учениками (требует проверки статуса назначений).
