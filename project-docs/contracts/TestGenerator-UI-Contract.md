...
### Preconditions
- Проект настроен для использования указанных CSS-фреймворков (например, Tailwind CSS).
- Шрифты (Inter, Roboto) доступны и подключены к проекту.
- Система иконок (Heroicons) интегрирована или доступна для использования.
- Разработчики ознакомлены с данным UI контрактом.

### Postconditions
- Весь пользовательский интерфейс генератора тестов реализован с использованием определенных в контракте цветов, типографики, отступов и компонентов.
- Достигнут консистентный визуальный стиль во всех разделах приложения.
- Реализованные компоненты соответствуют приведенным примерам кода и описаниям поведения.
- Приложение соответствует требованиям адаптивности и доступности, описанным в контракте.

### Invariants
- **Соответствие гайдлайнам**: Все элементы интерфейса (цвета, шрифты, отступы, компоненты) **всегда** соответствуют спецификациям, описанным в этом документе.
- **Консистентность**: Одинаковые элементы интерфейса (например, кнопки 'primary', заголовки карточек) выглядят и ведут себя одинаково во всем приложении.
- **Адаптивность**: Интерфейс корректно отображается и функционирует на всех определенных контрольных точках (breakpoints).
- **Доступность**: Приложение постоянно соответствует указанным стандартам доступности (WCAG AA, клавиатурная навигация, ARIA).
- **Целостность компонентов**: Используемые UI-компоненты (кнопки, формы, карточки и т.д.) сохраняют свою структуру и базовое поведение, определенное в контракте.

### Exceptions
- **Отклонение от гайдлайнов**: Создание UI элементов, не соответствующих определенным цветам, шрифтам, отступам или спецификациям компонентов.
- **Неконсистентность**: Различное отображение или поведение одинаковых по смыслу элементов в разных частях приложения.
- **Проблемы адаптивности**: Некорректное отображение или функционирование интерфейса на определенных размерах экрана.
- **Нарушение доступности**: Элементы не доступны с клавиатуры, недостаточный контраст, отсутствие ARIA-атрибутов и т.д.
- **Некорректная реализация компонента**: Компонент реализован с ошибками, не соответствует примерам кода или описанию поведения.
- **Отсутствие необходимого компонента**: Попытка использовать компонент, который не описан в UI контракте, без предварительного согласования и добавления.
