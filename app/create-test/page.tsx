...
const mockTableColumns = [
  { key: "orthography", label: "Орфография", type: "percentage" as const },
  { key: "punctuation", label: "Пунктуация", type: "percentage" as const },
  { key: "grammar", label: "Грамматика", type: "percentage" as const },
  { key: "lexicon", label: "Лексика", type: "percentage" as const },
  { key: "stylistics", label: "Стилистика", type: "percentage" as const },
  { key: "essay", label: "Сочинение", type: "percentage" as const },
  { key: "exposition", label: "Изложение", type: "percentage" as const },
  { key: "dictation", label: "Диктант", type: "percentage" as const },
  { key: "testing", label: "Тестирование", type: "percentage" as const },
  { key: "final", label: "Итоговая", type: "percentage" as const }
];

const mockTableStudents = mockStudentsData.map(student => ({
  id: student.id,
  name: student.name,
  email: student.email,
  score: student.score
...
const mockTableValues = mockStudentsData.map(student => 
  mockTableColumns.map(column => `${student.subjects[column.key as keyof typeof student.subjects]}%`)
);

// Convert mockTableValues to the expected data format for ResponsiveStatsTable
const mockTableData = mockStudentsData.reduce((acc, student, index) => {
  const studentData: Record<string, any> = {};
  mockTableColumns.forEach((column, colIndex) => {
    studentData[column.key] = student.subjects[column.key as keyof typeof student.subjects];
  });
  acc[student.id] = studentData;
  return acc;
  ...
  return (
    <div className="min-h-screen bg-background">
      <HeaderOrganism 
        ...
        breadcrumbItems={[{ label: "Главная", href: "/" }, { label: "Генерация теста" }]}
      />
